...
public class Line extends Obstacle implements Serializable {
    ...
    public double getCapteurValue(Capteur c){
        ...
        if(isCollideWith(c)){
            ...
            System.out.println("Point de colision: " + pointDeCollision);

            //Calcule de la valeur du capteur
            double longueurMax = Capteur.CAPTEUR_LENGHT;
            double longueurCollision = Math.sqrt(Math.pow(Math.abs(xCollide) - start.x, 2) + Math.pow(Math.abs(yCollide) - start.y, 2));

            return longueurMax / longueurCollision;
        }
        else{
            //Si aucune collision
            return -1.0;
